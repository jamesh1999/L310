...
def rk4(current_pose, t, dt):
  ...
  def dx(t, y): return u * np.cos(yaws[t])
  def dy(t, y): return u * np.sin(yaws[t])
  def dyaw(t, y): return np.cos(np.floor(t))

  def inner(f, y_last, t, dt):
    k1 = f(t, y_last)
    k2 = f(t + dt/2, y_last + dt * k1/2)
    k3 = f(t + dt/2, y_last + dt * k2/2)
    k4 = f(t + dt,   y_last + dt * k3)

    return y_last + dt * (k1 + 2*k2 + 2*k3 + k4) / 6

  # Yaws necessary for further calculation by RK4
  yaws[t] = current_pose[YAW]
  yaws[t + dt/2] = inner(dyaw, yaws[t], t, dt/2)
  yaws[t + dt] = inner(dyaw, yaws[t + dt/2], t + dt/2, dt/2)

  # Next pose by RK4
  next_pose[X]   = inner(dx,   current_pose[X],   t, dt)
  next_pose[Y]   = inner(dy,   current_pose[Y],   t, dt)
  next_pose[YAW] = yaws[t + dt]
