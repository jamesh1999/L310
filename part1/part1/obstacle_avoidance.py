...
def braitenberg(front, front_left, front_right, left, right):
    ...
    # Solution:
    close_dist = safety_dist
    far_dist = 2

    # Scale to close_dist=-1, far_dist=1
    norm_dist = lambda d: 2 * (d - close_dist) / (far_dist - close_dist) - 1
    # Sigmoid
    sensor_act = lambda d: 1 / (1 + np.exp(-d))

    act_front = sensor_act(norm_dist(front))
    act_front_left = sensor_act(norm_dist(front_left))
    act_front_right = sensor_act(norm_dist(front_right))
    act_left = sensor_act(norm_dist(left))
    act_right = sensor_act(norm_dist(right))

    forwards_acc = 3/4 * act_front + 1/8 * act_front_left + 1/8 * act_front_right
    sideways_acc = act_left + act_front_left / 3 - act_front_right / 3 - act_right
    act_sideways = sensor_act(-sideways_acc)

    u = max_u * (1 - forwards_acc)
    w = 2 * max_w * (act_sideways - 0.5)
    u = max_u
    return u, w
